...
Follows these steps to clone the respository:

1) Intsall git lfs if it is not installed already. This is needed here because the repository contains data files upload    through git lfs which can be viewed in the original form only if git lfs is installed.
